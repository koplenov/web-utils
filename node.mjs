...
;
	($.$koplenov_utils) = class $koplenov_utils extends ($.$mol_page) {
		counter_block_title(){
			return "";
		}
		...
		Counter_block(){
			...
			(obj.content) = () => ([(this?.Input_text_in_counter_block())]);
			return obj;
		}
		plugins(){
			return [];
		...
		body(){
			return [(this?.Counter_block())];
		}
	};
	($mol_mem(($.$koplenov_utils.prototype), "Input_text_in_counter_block"));
	($mol_mem(($.$koplenov_utils.prototype), "Counter_block"));
